...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'alamin-hosting-render-secret-key-2026-CHANGE-ME')
...
# ডিফল্ট অ্যাডমিন পাসওয়ার্ড (environment variable থেকে)
DEFAULT_ADMIN_PASSWORD = os.environ.get('ADMIN_PASSWORD', 'mdalaminmmmnnn037@gmail.com')

# 🔐 সার্ভার তৈরি করার API key (Render Environment-এ সেট করুন)
CREATE_API_KEY = os.environ.get('CREATE_API_KEY', 'ALAMIN-SECRET-KEY-2026')
...
def hash_password(password):
    """SHA-256 + salt দিয়ে পাসওয়ার্ড হ্যাশ"""
...
def verify_password(password, stored):
    """পাসওয়ার্ড যাচাই — পুরনো plain text এবং নতুন hash দুটোই সাপোর্ট করে"""
    if not stored:
        return False
    if '$' not in stored:
...
# ============================================
# 🔥 পাবলিক API - সার্ভার তৈরি (API Key Protected)
# ============================================
...
def api_create_server():
    ...
    if not provided_key or provided_key != CREATE_API_KEY:
        return jsonify({
            'status': 'error',
            'message': 'Unauthorized! Valid api_key is required.'
        }), 401

    # 🔐 Step 2: username & password must be provided
    ...
    try:
        ...
        return jsonify({'status': 'error', 'message': 'CPU and days must be numbers!'}), 400
...
# ============================================
# অ্যাডমিন পাসওয়ার্ড পরিবর্তনের API
# ============================================
...
# ============================================
# GitHub API (Git ছাড়া Python Download)
# ============================================
...
if __name__ == '__main__':
    ...
    print(f"📍 Admin password: {'[FROM ENV]' if os.environ.get('ADMIN_PASSWORD') else '[DEFAULT]'}")
    print(f"📍 Create API key: {'[FROM ENV]' if os.environ.get('CREATE_API_KEY') else '[DEFAULT]'}")
    print("=" * 50 + "\n")
